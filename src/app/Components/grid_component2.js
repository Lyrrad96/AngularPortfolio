...
app.controller('AGGridController', function ($scope, $rootScope, $timeout, $compile, uiGridConstants, uiGridTreeBaseService) {
	...
	console.log($scope.agconfig.scope);
	...
	function hideMenu() {
		...
		// document.getElementById("headerContextMenu").style.display sd= "none"
	}
	...
	var enableTextFilter = () => {
		$(document).ready(function(){
			...
			$('[ref=eInput][type=text][disabled]').each((i, elem)=>{
				...
				var id = $scope.gridOptions.columnApi.getAllColumns()[findAllByKey(elem, 'parentNode', 'attributes["aria-colindex"]?.value')-1].colDef.field
...
// 			console.log('%cgrid_component.js line:957 params.data.children', 'color: #007acc;', params.data);
// 			if (params.data.children?.length > 0) {
// 				if (params.data.expanded) {
// 					this.eGui.innerHTML = `<i class="fa fa-minus"></i> ` + params.value;
// 				} else {
// 					this.eGui.innerHTML = `<i class="fa fa-plus"></i> ` + params.value;
// 				}
// 			} else {
// 				this.eGui.innerHTML = params.value;
// 			}
// 			this.eventListener = function () {
// 				console.log('%cgrid_component.js line:967 params.data.rowId', 'color: #007acc;', params.data.rowId);
// 				updateData(params.data.rowId);
// 			};
...
// 	}
	...
	console.log('%cgrid_component.js line:728 $scope.agconfig', 'color: #007acc;', $scope.agconfig.rowData);
	let idCount = 0;
	// $timeout(()=>{
		// addIdInDataResurcive(rowData);
	// 	$scope.gridOptions?.scope?.gridOptions?.api?.setRowData(rowData);
	// 	$scope.gridOptions?.scope?.gridOptions?.api?.setColumnDefs(columnDefs);
	// }, 100)
	...
	function updateData(rowId) {
		let foundData = recursiveFindById($scope.agconfig.rowData, rowId);
		// console.log('%cgrid_component.js line:993 foundData', 'color: #007acc;', foundData);
		if (foundData) {
		foundData.expanded = !foundData.expanded;
		rowDataExpanded = [];
		makeDataResurcive($scope.agconfig.rowData, 0);
		$scope.gridOptions.api.setRowData(rowDataExpanded);
		// gridOptions.api.sizeColumnsToFit();
		}
	}

	function recursiveFindById(arrayData, rowId) {
		// console.log('%cgrid_component.js line:1005 arrayData', 'color: #007acc;', arrayData);
		let foundData = arrayData.find(e => e.rowId === rowId);
		...
		arrayData.every(e => {
			if(e.children)
				foundData = recursiveFindById(e.children, rowId);
			if (foundData) {
				return false;
			}
		return true;
	...
	function makeDataResurcive(arrayData, level) {
		// console.log('%cgrid_component.js line:1022 arrayData, level', 'color: #007acc;', arrayData, level);
		arrayData.forEach(mainRow => {
		mainRow.level = level;
		/* if (
			mainRow.children?.length > 0 &&
			mainRow.expanded &&
		...
		if (mainRow.expanded && mainRow.children) {
			makeDataResurcive(mainRow.children, level + 16);
		}
		});
	}

	function addIdInDataResurcive(arrayData) {
		// console.log('%cgrid_component.js line:784 arrayData', 'color: #007acc;', arrayData);
		arrayData.forEach(mainRow => {
		mainRow.rowId = idCount;
		idCount++;
		addIdInDataResurcive(mainRow.children);
		});
	}
	...
	function openTree(n) {
		updateData(0)
		for(var i = 0; i < n; i++) {
			console.log('%cgrid_component.js line:1053 i', 'color: #007acc;', i);
			$scope.agconfig.rowData/* rowDataExpanded */.map(row=>{
				// console.log('%cgrid_component.js line:1054 row.level', 'color: #007acc;', row, row.level, row.expanded, $scope.agconfig.rowData);
				if(!row.expanded)
					updateData(row.rowId)
			})
		}
		// console.log('%cgrid_component.js line:1060 $scope.agconfig.rowData', 'color: #007acc;', rowDataExpanded);
	}
	...
	$scope.mainSelection = (type, ev) => {
		...
		$timeout(()=>{
			switch (type) {
				...
				// 	})
				case 'custom':
					$scope.selectCustom(ev.rows, ev.columns)
					break;
				default:
					break;
			}
	...
	$scope.selectCustom = (rows, columns) => {
	rows.map(rid=>{
		var rowNode = $scope.gridOptions.api.getRowNode(rid)
		columns.map(cid=>{
			var newcell = {
				colId: cid,
				rowId: rid,
				rowInd: rowNode.rowIndex,
				rowNode: rowNode
			}
	...
	$scope.addRow = (ind, data, type) => {
		...
		data['newrow'] = true
		// $scope.agconfig.headerList.map(col=>data[col.colId]={v: ''})
		...
		// $scope.lastNodeId = $scope.gridOptions.api.getDisplayedRowAtIndex(ind).id
		// $scope.gridOptions.api.forEachNodeAfterFilter((node) => {
		// 	if($scope.lastNodeId < node.id){
		// 		console.log('%cgrid_component.js line:111 $scope.lastNodeId, node.id', 'color: #007acc;', $scope.lastNodeId, node.id, parseInt(node.id));
	...
	$scope.hideColumn = (ev, visible) => {
		// ev.preventDefault()
		// var columnDefs = $scope.gridOptions.api.getColumnDefs()
		var allCols = $scope.agconfig.headerList //$scope.gridOptions.columnApi.getAllColumns()
	...
	$scope.insertColumn = (ev, id, pos='right') => {
		...
		$scope.gridOptions.api.setColumnDefs(columnDefs)
		$scope.agconfig.headerList.push($scope.gridOptions.columnApi.getColumn([colname]))
	}
	$scope.deleteColumn = (ev, id) => {
		...
		var columnDefs = $scope.gridOptions.api.getColumnDefs()
		console.log('%cgrid_component.js line:89 ev', 'color: #007acc;', ev, colname, typeof ev/* , ev.parentNode, ev.parentNode */, $scope.agconfig.headerList.indexOf(colname));
		deleteCol(columnDefs, colname)
		$scope.agconfig.headerList.splice($scope.agconfig.headerList.findIndex(c=>c.colId==colname), 1)
...
$scope.fun = (f) => console.log('%cgrid_component.js line:1608 f', 'color: #007acc;', f);
	...
	$scope.splitCells = (type) => {
		switch (type) {
			case 'hyphen':
				...
				// var cols = [...$scope.agconfig.headerList].splice($scope.agconfig.headerList.findIndex(h=>h.colId == cellMerge.colId), cellMerge.data[0].length)
	...
	$scope.togglecb = (ele, type) => {
		...
		switch(type) {
			...
			case 'folded':
				console.log('%cgrid_component.js line:2310 ele, type', 'color: #007acc;', ele, type);
				updateData(ele);

				// ele.data.folded = !ele.data.folded
				// $scope.gridOptions.api.onFilterChanged();
		}
		// $scope.gridOptions.api.refreshCells({
		// 	force: true,
		// 	columns: ['check'],
		// });
		console.log('%cgrid_component.js line:2335 $scope.agconfig.selectedRows togglecb', 'color: #007acc;', $scope.agconfig.selectedRows);
	}
	...
	var defaultColDef = {
		...
		cellStyle: cellStyle,
		cellClassRules: {
			'selected': (params)=>$scope.agconfig.selectedCells.find(cell=>params.colDef.field == cell.colId && params.node.id == cell.rowId)&&!$scope.agconfig.rangeSelectedCells.length,
		...
		cellRenderer: function (params) {
			// console.log('%cgrid_component.js line:60 params', 'color: #007acc;', params, params.value, params.column.colId, $scope.agconfig.headerList, params.column.instanceId, params.value?.ct, params.data, params.$scope.$parent.$parent.$parent.togglecb);
			if(params.value?.v==undefined) return ''
			else if(params.value?.ct)
				return eval(params.value?.ct)
			else if(params.column.instanceId == 0 && $scope.agconfig.type=='tree')
					// if (params.data.expanded) {
					// 	return `<i class="fa fa-minus"></i> ` + params.value.v;
					// } else {
					// 	return `<i class="fa fa-plus"></i> ` + params.value.v;
					// }
					//
				return $compile(
					`
					<div style="width: 100%; padding-left: ${params.data.level}px" ng-click="togglecb(${params.data.rowId||0}, 'folded')">
					${params.data.hasOwnProperty('children')?`
						<span ng-show="this.data.hasOwnProperty('children')">
						${params.data.expanded ? $scope.agconfig.treeicon?.close||'<i class="fa fa-minus"></i>' : $scope.agconfig.treeicon?.open||'<i class="fa fa-plus"></i>'}
						</span>` : '' }
						${params.value?.v}
					</div>`
					)($scope)[0]
				...
				// return $compile(`
				// 	<div style="width: 100%; padding-left: ${params.data.parentId == null ? 0 : 16*params.data.level+20}px" ng-click="togglecb(this, folded)">
				// 		<span>
				// 			${params.data.parentId == null ? params.data.folded ? `<i class="fa fa-chevron-up"></i>` : `<i class="fa fa-chevron-down"></i>` : ''}
				// 		</span>
				// 		${params.value?.v}
		...
		onCellClicked: (params) => {
			// console.log('%cgrid_component.js line:2522 params.rowIndex', 'color: #007acc;', params.rowIndex);
			// if(params.column.instanceId == 0 && params.data.children?.length > 0 && $scope.agconfig.type=='tree')
			// 	$scope.togglecb(params, 'folded')
		},
    ...
    $scope.gridOptions = {
		...
		floatingFilter: $scope.agconfig.filter,
		...
		doesExternalFilterPass: function(rowNode) {
			...
			if($scope.selectionFilter?.length) {
				// console.log('%cgrid_component.js line:1466 $scope.selectionFilter', 'color: #007acc;', Object.values(rowNode.data), $scope.selectionFilter?.toLowerCase(), $scope.agconfig.headerList.find(h=>rowNode.data[h.colId]?.toLowerCase().includes($scope.selectionFilter?.toLowerCase())), rowNode, $scope.agconfig.selectedCells.find(c=>c.rowNode==rowNode));
				...
				// flg = $scope.agconfig.headerList.find(h=>rowNode.data[h.colId]?.toLowerCase().includes($scope.selectionFilter?.toLowerCase())) && $scope.agconfig.selectedCells.find(c=>c.rowNode==rowNode) && flg
			}
			// console.log('%cgrid_component.js line:1104 rowNode.data.hidden', 'color: #007acc;', rowNode.data.hidden);
			flg = flg && !rowNode.data.hidden
			if($scope.agconfig.type == 'tree'){
				// console.log('%cgrid_component.js line:2214 rowNode', 'color: #007acc;', rowNode, rowNode.data.folded, $scope.gridOptions.api.getRowNode(rowNode.data.parentId));
				flg = flg && !$scope.gridOptions.api.getRowNode(rowNode.data.parentId)?.data?. folded/*  || rowNode.data.parentId !== null */
        ...
        onSelectionChanged: (param) => {
			...
			// $scope.agconfig.selection = $scope.agconfig.selectedRows.map(row=>$scope.agconfig.headerList.map(head=>row[head.colId]?.v || ''))
		...
		getRowStyle: (params) => {
			...
			// 	addObj([params.data, 'rowStyle'], {'background-color': '#d6d6f3'})
			// if (params.node.id==$scope.lastNodeId) 
			// 	addObj([params.data, 'rowStyle'], {'background-color': '#e1e8f9'})
        ...
        onGridReady: function(params) {
        	// sizeToFit()
			// $scope.gridOptions.rowData = $scope.agconfig.rowData
			...
			// console.log('%cgrid_component.js line:79 checkSelect, addRow, addCol, delRow', 'color: #007acc;', checkSelect, addRow, addCol, delRow, params, $scope.columns, columnDefs, params, $scope.gridOptions, $scope.agconfig);
			// $scope.agconfig.headerList = $scope.gridOptions.columnApi.getAllDisplayedColumns()
			// $scope.viewAllCols = true
			// // $scope.colvis = Array.from({length: $scope.agconfig.headerList.length}, val=>true)
			// if(type == 'tree') {
			// 	console.log('%cgrid_component.js line:2750 $scope.agconfig.rowData', 'color: #007acc;', $scope.agconfig.rowData);
			// 	openTree($scope.agconfig.treeDefaultOpen)
			// 	columnDefs.unshift({
			// 		field: 'treearrow',
			// 		maxWidth: 50,
			// 		suppressMenu: true,
			// 		filter: '',
			// 		cellRenderer: (params) => {
			// 			// console.log('%cgrid_component.js line:383 params', 'color: #007acc;', params, params.api.getDisplayedRowAtIndex(params.rowIndex+1).data.parentId, params.data.id);
			// 			if(/*params.data?.type == 'parent'  && params.column.colId == 'treearrow' */params.api.getDisplayedRowAtIndex(params.rowIndex+1)?.data.parentId==params.data.rowId) return params.data.folded ? `<i class="fa fa-chevron-up"></i>` : `<i class="fa fa-chevron-down"></i>`
			...
			// 		// cellRenderer: (params)=> (params.data.rowId)?.toString()
			// 	})
			// }
	...
	$scope.refreshCells = (cid, rid) => {
		// console.log('%cgrid_component.js line:2484 cid, rid', 'color: #007acc;', cid, rid, $scope.refreshCells.caller);
		var params = {
			force: true,
		}
		if(cid.length)
	...
	$scope.updateGrid = (rowData, columnDefs) =>{
		if(rowData) {
			$scope.agconfig.rowData = rowData
			$scope.gridOptions.api.setRowData($scope.agconfig.rowData);
		}
		if(columnDefs) {
			$scope.agconfig.columnDefs = columnDefs
			$scope.gridOptions.api.setColumnDefs($scope.agconfig.columnDefs);
			$scope.agconfig.headerList = $scope.gridOptions.columnApi.getAllDisplayedColumns()
		}
		$scope.initGrid($scope.gridOptions)
	}
	...
	$scope.initGrid = (config) => {
		...
		var columnDefs = $scope.gridOptions.api.getColumnDefs()
		$scope.agconfig.rowNo = $scope.gridOptions.api.getDisplayedRowCount()
		console.log('%cgrid_component.js line:79 checkSelect, addRow, addCol, delRow', 'color: #007acc;', checkSelect, addRow, addCol, delRow, columnDefs, $scope.gridOptions, $scope.agconfig);
		// $scope.agconfig.headerList = $scope.gridOptions.columnApi.getAllDisplayedColumns()
		$scope.viewAllCols = true
		// $scope.colvis = Array.from({length: $scope.agconfig.headerList.length}, val=>true)
		if(type == 'tree') {
			...
			// 	// editable: false,
			// })
			console.log('%cgrid_component.js line:2396 $scope.agconfig.headerList', 'color: #007acc;', $scope.agconfig.headerList);
		}
		if(slno && !columnDefs.find(col=>col.field=='slno')) {
			columnDefs.unshift({
				field: 'slno',
				headerName: "Sl No",
				// headerName: "id",
				...
				headerClass: headerRowspanClass(0, 0, headerRowNo) + ' jcenter',
				// cellRenderer: (params)=> params.rowIndex + 1
				cellRenderer: (params)=> /* params.data.rowId?.toString() ||  */params.rowIndex + 1
			})
		}
		// Adds column on clicking + icon in header
		if(addCol && !columnDefs.find(col=>col.field=='addCol')) {
			console.log('%cgrid_component.js line:181 addCol', 'color: #007acc;', addCol);
			columnDefs.unshift({
				field: 'addCol',
				headerName: 'Add Column',
		...
		// if(params.data?.header) {
			...
			if(checkSelect && !columnDefs.find(col=>col.field=='check')) {
				console.log('%cgrid_component.js line:45 $scope.gridOptions', 'color: #007acc;', $scope.gridOptions, columnDefs);
				var col = 'blue'
				columnDefs.unshift({
					field: 'check',
			...
			if(merge && !columnDefs.find(col=>col.field=='merge')) {
				console.log('%cgrid_component.js line:45 $scope.gridOptions', 'color: #007acc;', $scope.gridOptions, columnDefs);
				var col = 'blue'
				columnDefs.unshift({
					field: 'merge',
			...
			// Adds new row below on clicking + icon
			if(addRow && !columnDefs.find(col=>col.field=='addRow')) {
				columnDefs.unshift({
					field: 'addRow',
					headerName: 'Add',
					headerClass: headerRowspanClass(0, 0, headerRowNo),
			...
			if(addDup && !columnDefs.find(col=>col.field=='addDup')) {
				columnDefs.unshift({
					field: 'addDup',
					headerName: 'Add',
					headerClass: headerRowspanClass(0, 0, headerRowNo),
			...
			// Deletes row on clicking trash icon
			if(delRow && !columnDefs.find(col=>col.field=='delRow')) {
				console.log('%cgrid_component.js line:82 columnDefs', 'color: #007acc;', columnDefs);
				columnDefs.push({
					field: 'delRow',
					headerName: 'Delete',
		...
		if(colSelect) {
			...
			$scope.gridOptions.defaultColDef['headerComponentParams'] = {
				template:`
					<div class="ag-cell-label-container" role="presentation" onclick="angular.element('.ag-theme-alpine').scope().mainSelection('col', event)">
					<!--<div class="ag-cell-label-container" role="presentation" ng-click="selectColumn($event)">-->
						<!--<span onclick="angular.element('.ag-theme-alpine').scope().addColumn(this)"><i class="feather icon-plus" style="cursor: pointer"></i></span>-->
						<span ref="eMenu" class="ag-header-icon ag-header-cell-menu-button"></span>
						<div ref="eLabel" class="ag-header-cell-label" role="presentation">
		...
		// Row drag
		console.log('%cgrid_component.js line:2639 ', 'color: white; background-color: #007acc;', new Date());
		if($scope.agconfig.rowDragManaged && !columnDefs.find(col=>col.field=='rowDrag')){
			columnDefs.unshift({
				field: 'rowDrag',
				// editable: false,
				// suppressMovable: true,
		...
		$timeout(()=>{
			...
			.mousedown(function(event) {
				...
				if(!$scope.agconfig.headerList.filter(h=>h.colId == focusedCell.column.colId).length) {
					// console.log('%cgrid_component.js line:1994 $scope.agconfig.headerList.filter(h=>h.colId == c)', 'color: #007acc;', $scope.agconfig.headerList.filter(h=>h.colId == focusedCell.column.colId));
				...
				// console.log('%cgrid_component.js line:1433 isDragging', 'color: #007acc;', isDragging, event, focusedCell, range.startRow, range.startColumn, range.endRow, range.columns, len, $scope.agconfig.headerList.filter(h=>h.colId == focusedCell.column.colId), $scope.gridOptions.api.getDisplayedRowAtIndex(focusedCell.rowIndex));
				range.startRow = {
				...
				.mousemove(function(event) {
					...
					for(var i = 0, j = c > ind ? ind : c - 1; c > ind ? j < c : j <= ind && ind != -1; i++, j++) {
						// console.log('%cgrid_component.js line:1469 i', 'color: #007acc;', i, c, ind, i-c, Math.abs(i - c), Math.abs(ind - c), $scope.agconfig.headerList, columns[j], columns);
				...
				.mouseup(function(event) {
					...
					if(event.shiftKey) {
						...
						range.columns = $scope.agconfig.headerList.slice($scope.agconfig.headerList.findIndex(h=>$scope.agconfig.selectedCells[0].colId==h.colId), $scope.agconfig.headerList.findIndex(h=>findAllByKey(event.target, 'parentNode', 'attributes?.["col-id"]?.value')==h.colId)+1)
						$scope.agconfig.rangeSelectedCells[0] = range
						// console.log('%cgrid_component.js line:2892 range', 'color: #007acc;', range, $scope.agconfig.headerList.indexOf($scope.gridOptions.api.getFocusedCell().column), $scope.agconfig.headerList.findIndex(h=>findAllByKey(event.target, 'parentNode', 'attributes?.["col-id"]?.value')==h.colId));
						$scope.refreshCells('all')
					}
					...
					// console.log('%cHello grid_component.js line:2446 ', 'background: green; color: white; display: block;', !event.ctrlKey, !event.shiftKey, event.button == 0, mouseEnd.x, mouseStart.x, mouseEnd.y, mouseStart.y, $scope.agconfig.type=='tree', focusedCell.column.colId, $scope.agconfig.headerList[0].colId);
					// if(!event.ctrlKey)
					...
					if(!event.ctrlKey && !event.shiftKey && event.button == 0) {
						// $scope.agconfig.selectedCells = []
						// $scope.agconfig.rangeSelectedCells = []
						if(mouseEnd.x==mouseStart.x&&mouseEnd.y==mouseStart.y&&($scope.agconfig.type=='tree'&&focusedCell.column.colId!=$scope.agconfig.headerList[0].colId)) {
							deselectAll('all')
							var newcell = {
...
app.directive('agGridData', function () {
	return {
		restrict: 'AE',
		template: `
			<div ng-if="agconfig.toolbar" class="toolbar">
					...
					<!--ul class="nested_dropdown">
						<li>
							...
							<ul>
								...
								<li ng-repeat="header in agconfig.headerList track by $index">
			...
			<div ng-style="{'height': 'calc(100% - ' + (agconfig.toolbar ? '60px' : '-20px') + ')'}">
			...
			<style>
				...
				*/
				...
				.selected {background-color: #F4F3FF; color: #6938EF}
